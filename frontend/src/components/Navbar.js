  ...
  return (
    <div>
      ...
      <div className="navbar">
        <i ref={menuIconRef} className="bx bx-menu" onClick={handleMenuClick}><IoReorderThreeSharp /></i>
        <div className="logo">
          <Link to="/">
            <img src={logo} alt="Logo" />
          </Link>
        </div>
        <div ref={navLinksRef} className="nav-links" style={{ left: navOpen ? '0' : '-100%' }}>
          <div className="sidebar-logo">
            <span className="logo-name">
              <img style={{ width: '150px' }} src="/src/Assets/logo.webp" alt="Logo" />
            </span>
            <i className="bx bx-x" onClick={handleCloseClick}><IoMdClose /></i>
          </div>
          <ul className="links">
            <li>
              <Link to="/">HOME</Link>
